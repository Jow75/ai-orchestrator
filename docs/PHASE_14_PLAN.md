...
Branch, dirty/clean state (both already shown by `/status` — reused, not reimplemented), and the last 5–10 commit subjects (`git log --oneline -N` equivalent), shelling out to the real `git` binary the same way `/status`'s existing `git.branch`/`git.dirty` fields already do. Read-only. No approval gate, matching `/status`. Kill switch: `operator.git.enabled`.
**Depends on:** nothing. **Risk:** low — reuses an existing, already-shipped git-shelling pattern.

### M2 — Log Visibility (`/log [project] [n]`)
...
### M3 — Repository & Symbol Search (`/grep <pattern> [project]`, `/symbol <name> [project]`)
...
**Depends on:** M1/M2 not required, but should ship after them — same review category, same review pass.
**Risk:** medium — the highest-blast-radius new surface this phase adds (arbitrary regex/text search over a real project's files), same risk class M6 treated `/files`/`/file` as. Needs the same adversarial test discipline M6's path-traversal suite established: searches must stay inside the project root, must not read binary files into a text response, must cap result count/response size the same way `/files` caps page size.
