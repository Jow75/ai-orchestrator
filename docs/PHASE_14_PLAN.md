...
### Cleanup pass — pre-M4 architecture review — SHIPPED, `v3.16.0`
Not a numbered milestone: a directed pass triggered by the owner asking, before M4 started, for a short review of M9/M0/M1/M2/M8/M3 together — specifically, what got duplicated across six milestones' worth of new commands, and what inconsistencies would be cheaper to fix now than after three more milestones build on the same pattern. Found two real duplication patterns (pagination math tripled across `fileAccess.js`/`repoSearch.js`/`logVisibility.js`; the "trailing bare number is a page" parse and the pagination footer text each tripled in `commandRouter.js`/`render.js`; kill-switch boilerplate repeated at a dozen call sites) and five inconsistencies (`/git` was the one surface that skipped `fileAccess.js`'s containment guard in favor of a bespoke `fs.existsSync()`; three different wordings for "this project's folder isn't on disk"; a literal string duplicated between `/whoami` and `activeProjectRoot()`; `/workspace`/`/git`/`/log` emitting no event on success, the one gap in this codebase's own router rule 4 among six milestones; `gitVisibility.js` naming its directory parameter `dir` where every other module calls the same concept `projectRoot`). All fixed — see `CHANGELOG.md`'s `v3.16.0` entry for the full list. No command's owner-visible behavior changed except `/git`'s folder-missing wording and the three new `*.viewed` events. 1304 → 1314 backend tests, zero regressions.

### M4 — TODO/FIXME Discovery (`/todos [project]`) — SHIPPED, `v3.17.0`
...
**Shipped shape (one expansion beyond the sketch above, one deviation from M3's own precedent):**
- **Nine annotation tags, not three** — `repoSearch.js`'s new `ANNOTATION_TAGS` covers `TODO`, `FIXME`, `BUG`, `HACK`, `XXX`, `NOTE`, `OPTIMIZE`, `REVIEW`, `DEPRECATED`, following an owner recommendation (made against the M3 milestone review) to make this real "code intelligence," not just the two best-known tags. Each result is labelled with the specific tag that matched (new `matchedTag()`). Deliberately case-sensitive — `NOTE`/`REVIEW`/`HACK`/`BUG` are all ordinary English words, and matching them case-insensitively would flag prose, not just annotations.
- **Accepts an optional `[project]` argument**, unlike `/grep`/`/symbol`. Those two dropped theirs because a free-text query and a free-text project name can't be told apart with no delimiter — the exact ambiguity M9 hit first (see M9's section above). `/todos` has no owner-typed query at all (the pattern is fixed), so that ambiguity never applies, and it resolves a named-or-active project exactly like `/git`/`/log` do, reading via `getRawProject()` so it works regardless of mission-readiness.
- Reuses `search.performed` (payload `mode: 'todos'`) rather than a new event type — the same "shared event, `mode` field distinguishes the command" pattern `/grep`/`/symbol` already established, since this is not a new capability.

`repoSearch.test.js` (+6), `commandRouter.test.js` (+14). 1314 → 1334 backend tests, zero regressions.
**Depends on:** M3. **Risk (realized):** low, as predicted — inherited M3's guard, added no new surface.

### M5 — Test & Verification Visibility (`/tests [project]`) — SHIPPED, `v3.19.0`
...
**Shipped shape (matches the sketch above with no deviation — the first Phase 14 milestone that didn't need one):** reads `this.taskQueue.load(project)`, the exact same persisted queue `/tasks` already reads — no new module, no new data source. One block per task (`T1 [done]:`) with each real verifier's ✅/❌ mark and its `detail` string underneath, so a failure reads as "file-exists — dist/bundle.js not found," not just a number. `marker`-type results are excluded via `missionCard.js`'s own exported `isEvidenceVerifier()` (imported into `render.js`, not reimplemented) — the same rule that keeps `missionCard`'s own aggregate honest applies here too: a completion marker is the agent's own claim about itself, not a check of it. Two distinct "nothing to show" cases, reported differently on purpose: no task queue at all ("no verification data yet") versus a real queue where nothing has recorded verifier evidence yet ("No completed task has recorded verifier results yet") — collapsing them would hide the difference between "never ran" and "ran but nothing checked it." New kill switch `operator.tests.enabled` (its own, not reusing `operator.search` the way M4's `/todos` did — M5 is a genuinely different capability, not a pre-canned search). New event `tests.viewed`, matching the `workspace.viewed`/`git.viewed`/`log.viewed` read-only-visibility pattern the pre-M4 cleanup pass established.

`operatorRender.test.js` (+4), `commandRouter.test.js` (+7). 1376 → 1387 backend tests, zero regressions.
**Depends on:** nothing new architecturally; reads existing `checkpoint.js` data. **Risk (realized):** low, as predicted.

### M6 — AI-Assisted Engineering Mission Templates — SHIPPED, `v3.18.0`
...
- `/docgen <path>` — draft documentation for a specific file or module.
- `/refactor <description>` — a **proposal only** (the mission's objective is explicitly "propose a plan, do not implement it" for the first-pass version of this command; actually applying a refactor is just a normal mission request afterward, inheriting the standard two-gate approval before any write).
...
**Shipped shape (one deviation from the sketch above, forced by the same problem M3's own sketch ran into):** `/docgen` and `/refactor` do **NOT** accept `[project]` — a free-text path or free-text description can't be delimited from a free-text project name any more than M3's `/grep <pattern> [project]` could, so both dropped it and operate on the active project only, selected first with `/project <name>`, exactly like `/grep`/`/symbol`/`/files`/`/file` already do. `/review`/`/architecture` keep `[project]` — neither takes a second free-text argument, so the ambiguity never arises, and both resolve a project the same way `/git` does (`getRawProject()` + `resolveWithinProject(root, '')`), working uniformly regardless of mission-readiness. `src/operator/missionTemplates.js` (new) holds the four objective builders; `commandRouter.js`'s new `submitMissionTemplate()` is the one place a template becomes a real `MissionRequestStore` request, shared with `handleFreeText()` itself (refactored to call it, removing a third copy of the same five lines). No new event type: `mission.created` gained a `template` payload field (`'review'|'architecture'|'docgen'|'refactor'|null`) rather than inventing one, the same "shared event, one field" pattern `/grep`/`/symbol`/`/todos` already established for `search.performed`. New kill switch: `operator.missionTemplates.enabled`.

`missionTemplates.test.js` (new, 7 tests), `commandRouter.test.js` (+35). 1334 → 1376 backend tests, zero regressions.
...
**Risk (realized):** low-to-medium, as predicted — the risk isn't a new execution path (there isn't one), it's prompt quality (a bad canned template produces a bad mission, same as a bad free-form request would) — mitigated by treating each template as reviewed, versioned text, not user input, and by reusing M1's/M3's existing guards rather than adding new ones.

### M7 — Phase 12 M4 Re-evaluation (Launch Experience & Remote Project Creation) — DECIDED, 2026-08-01: deferred again
...
**The decision, and the evidence behind it.** The re-evaluation split M4's original scope into its two independent halves and checked each against everything Phase 13/14 actually shipped:

- **Launch experience** (launcher, Start Menu) — **substantially resolved by work that already shipped for other reasons.** The desktop app (Phase 12 M3) is a real GUI daemon client; the 2026-08-01 Task Scheduler console-window fix (`b51c7c3`, standalone, outside any milestone) makes autostart genuinely silent. What remains — a literal Start Menu shortcut — is minor UX polish, not a capability gap anything is currently blocked on.
- **Remote project creation** (`/new`) — **not covered by anything shipped**, and re-confirmed as the one real gap. `/scan` + `/import` + `/import all` (Phase 13 M2, reconciliation pass) register EXISTING folders; `/mission` + `/mission all` (Phase 14 M9) write a starter `promptFile` INTO an EXISTING folder missing one. Neither creates a new folder, `git init`s it, or scaffolds a starter file tree — `/new`'s actual job. This is qualitatively different new ground: every other Phase 14 milestone extended an existing read or mutate path (`fileAccess.js`'s guard, `configManager.updateProject()`, the mission-request pipeline); `/new` would be the first surface that creates new filesystem state from nothing, under a trust boundary (`operator.projectRoots`) nothing has fully exercised as a write target yet.

Weighed against the owner's own standing direction that Phase 14 is meant to be **the last infrastructure-heavy phase**, and the observation that no live Phase 13/14 review session ever surfaced an owner-felt need for remote project creation the way `/git`/`/log`/`/todos`/`/tests` each did (every one of those came from a real, disclosed gap found during live use) — **the owner's decision (2026-08-01) is to defer M4 again**, formally, rather than resume it narrow or full. Launch experience is treated as closed by the work that already shipped for other reasons; remote project creation stays out of Phase 14 and is not currently planned — it is picked up only if a real, owner-felt need for it surfaces, the same bar every other Phase 14 command had to clear. No version bump: like Phase 12's own M4 deferral note, a decision is not a code change and does not consume a `v3.x` slot.
...
**Risk:** N/A — this milestone was a decision point, not an implementation, and is now closed.
...
**Actual order deviated once:** M6 shipped immediately after M4, ahead of M5 (Test/Verification Visibility), per the owner's explicit 2026-08-01 M4 acceptance-review direction to go straight to "AI-Assisted Mission Templates" next. M6 had no hard dependency on M5 to begin with (see the dependency graph above — M5 is listed independent), so nothing about M6's own implementation changed; M5 remains open and unblocked by this.
