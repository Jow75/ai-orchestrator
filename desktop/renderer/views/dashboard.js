  ...
  function html(status, projects, selectedProject) {
    ...
    const cards = !status ? OViz.empty('No status yet — start a mission from the Missions tab.') : `
      <div class="grid">
        ...
        <div class="card"><h3>Agent</h3><div class="value" style="font-size:15px">${OViz.escapeHtml(mission?.currentAgent ?? agent?.driver ?? '—')}</div><div class="sub">${mission?.currentAgentRole ? OViz.escapeHtml(mission.currentAgentRole) + ' · ' : ''}${agent?.pid ? `pid ${agent.pid}` : OViz.escapeHtml(agent?.driver ?? '')}</div></div>
